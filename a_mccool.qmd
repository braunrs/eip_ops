...
enc$approach_assessment[(enc$counselor_risk_start == 'zmccan0w') &
...
# Declined
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments_declined[(enc$counselor_risk_start == 'zmccan0w') &
...
# Prevented
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments_prevented[(enc$counselor_risk_start == 'zmccan0w') &
                            enc$offer_outcome_risks == 2] <- 1
...
# Assessed
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments[(enc$counselor_risk_start == 'zmccan0w') &
...
table(enc$ctr_calc)
enc$ctr_tests[enc$ctr_calc == 1 & enc$counselor_hiv_start == 'zmccan0w'] <- 1
...
table(enc$ehe_calc)
enc$ehe_tests[enc$ehe_calc == 1 & enc$counselor_hiv_start == 'zmccan0w'] <- 1
ehe_tests <- enc[which(enc$ehe_calc == 1 & enc$counselor_hiv_start == 'zmccan0w'), ]
...
enc$hiv_false_positive[enc$counselor_hiv_start == 'zmccan0w' &
...
enc$hiv_indeterminate[enc$counselor_hiv_start == 'zmccan0w' & enc$final_test_status_hiv == 2] <- 1
...
enc$hcv_rapid_test[enc$counselor_hcv_start == 'zmccan0w' &
...
::: {.g-col-8}
This metrics report shows data for encounter information for **Ashley McCool**. 
Ashley's start and end date with EIP are shown below, however, any data contained 
