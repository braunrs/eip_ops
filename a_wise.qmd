...
eip$appalachian_pt[eip$address_zip_code == 43006 | eip$address_zip_code == 43056 |
                     ...
                     eip$address_zip_code == 45788 | eip$address_zip_code == 45789] <- 1
...
eip$Wise[eip$counselor_enc_start == 'zwisa5yf' |
              eip$counselor_risk_start == 'zwisa5yf' |
              eip$counselor_hiv_start == 'zwisa5yf' |
              eip$counselor_hcv_start == 'zwisa5yf' |
              eip$enc_counselor == 94 |
              eip$ltc_coord_start == 'zwisa5yf' |
              eip$ltc_coordinator_per_case___94 == 1] <- 1
table(eip$Wise)
...
monthly_encounters <- select(enc, enc_date, Wise)
monthly_encounters <- monthly_encounters[which(monthly_encounters$Wise == 1), ]
...
enc$no_service[(enc$counselor_risk_start == 'zwisa5yf' | enc$enc_counselor == 94) &
...
enc$approach_assessment[(enc$counselor_risk_start == 'zwisa5yf' | enc$enc_counselor == 94) &
...
# Declined
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments_declined[(enc$counselor_risk_start == 'zwisa5yf' | enc$enc_counselor == 94) &
...
# Prevented
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments_prevented[(enc$counselor_risk_start == 'zwisa5yf' | enc$enc_counselor == 94) &
                            enc$offer_outcome_risks == 2] <- 1
...
# Assessed
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments[(enc$counselor_risk_start == 'zwisa5yf' | enc$enc_counselor == 94) &
...
enc$sud_assessed[enc$assessments == 1 &
                   ((enc$audit_c1 == 0 | enc$audit_c1 == 1 | enc$audit_c1 == 2 | enc$audit_c1 == 3 |
                      ...
                      (enc$other_drug_use == 0 | enc$other_drug_use == 1 | enc$other_drug_use == 2 |
                         enc$other_drug_use == 3 | enc$other_drug_use == 4))] <- 1
...
table(enc$ctr_calc)
enc$ctr_tests[(enc$counselor_risk_start == 'zwisa5yf' | enc$enc_counselor == 94) & enc$ctr_calc == 1] <- 1
...
table(enc$ehe_calc)
enc$ehe_tests[(enc$counselor_risk_start == 'zwisa5yf' | enc$enc_counselor == 94) & enc$ehe_calc == 1] <- 1
...
enc$hiv_false_positive[(enc$counselor_risk_start == 'zwisa5yf' | enc$enc_counselor == 94) &
...
enc$hiv_indeterminate[(enc$counselor_risk_start == 'zwisa5yf' | enc$enc_counselor == 94) &
...
enc$hcv_rapid_test[(enc$counselor_risk_start == 'zwisa5yf' | enc$enc_counselor == 94) &
...
enc$ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
                (enc$ltc_services___1 == 1 |
                   enc$ltc_services___2 == 1 |
                   enc$ltc_services___3 == 1 |
                   enc$ltc_services___4 == 1)] <- 1
table(enc$ltc_cases)
...
enc$hiv_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$hcv_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$sud_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$ht_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$mh_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$housing_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$prep_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$sti_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$food_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$financial_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$personabuse_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$general_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
enc$other_ltc_cases[enc$ltc_coordinator_per_case___94 == 1 &
...
::: {.g-col-8}
This metrics report shows data for encounter information for **Alyssa Wise**. The metrics in this report describe specific encounter information and the services and/or referrals that were provided to patients/clients.
...
Alyssa has had a total of **`r encounters` encounters** entered into the EIP Database since January 1st, 2023. A summary table of these encounters and what they consisted of are shown below.
...
The graph below shows the total number of encounters Alyssa has had during each month within the reporting period. Also, the total number of each service provided are shown by the trend lines, which allows you to see trends over time. The definitions for how each of these numbers are stratified is described below the graph. This plot is dynamic, meaning you can select and deselect each particular metric by clicking the text in the legend on the right to focus on particular services provided. The plot also has the ability to zoom in on particular time frames by dragging the pointer to select the desired timeframe, and you can also scroll through timepoints using the slider below the x-axis. You can reset the graph by double-clicking within the plot. As mentioned above, not all staff offer or perform these services, so you can deselect the ones not within Alyssa's role for a more tailored focus of tasks. This is a visual representation of the data shown in the complete summary metrics table above.
...
metrics_plot <- metrics_plot %>%
  layout(title = 'Wise Monthly Encounters & Services',
...
The table shown below shows the average monthly numbers for encounters and each type of service noted above. This is the average number of encounters/services based on the time frame shown in the graph above, which is when Alyssa started documenting encounters/services with EIP.
...
assessment_plot <- assessment_plot %>%
  layout(title = 'Wise Monthly Assessment Trends',
         #xaxis = list(rangeslider = list(type = "date")),
         yaxis = list(title = "Total")) %>%
  config(displayModeBar = FALSE)
assessment_plot
...
The total number of HIV tests, including both CTR and EHE tests, are shown in the plot below. These only include rapid-based HIV tests that are performed by Alyssa, and does not include any lab tests that are initiated by medical staff prompted by EIP, or lab draws performed in the community (*unless it is the result of an initial rapid test*). The plot has the ability to select only CTR/EHE tests, and has the ability to zoom in on particular time frames by clicking and dragging. You can reset the graph by double-clicking within the plot.
...
monthly_hiv_testing_plot <- plot_ly(data = monthly_hiv_tests, x = ~Date, y = ~ctr_tests, type = 'bar', name = 'CTR Tests',
  ...
  layout(title = 'Wise Monthly Rapid HIV Testing',
         #xaxis = list(rangeslider = list(type = "date")),
...
Additionally, false positive tests occur when the individual has an initial reactive/positive test, but additional testing shows that their final status is actually a negative result. Alyssa has had a total of **`r hiv_false_positive` tests that have resulted in a false positive** (*`r hiv_false_positive_percent` of all HIV tests, `r hiv_false_positive`/`r total_hiv_tests`*), or those that had an initial reactive result but a final negative status.

There are also instances when the initial test may be reactive, but the confirmatory test is negative, leading to a discordant result that requires additional testing (*HIV RNA testing*). Until that RNA testing is completed, this is known as an Indeterminate result. Alyssa currently has a total of **`r hiv_indeterminate` tests that are indeterminate and in need of RNA testing** (*`r hiv_indeterminate_percent` of all HIV tests, `r hiv_indeterminate`/`r total_hiv_tests`*). This is counted when the final test status is noted as Indeterminate. When the RNA testing confirms the final result, the final status is updated. Therefore, this only counts those that are currently marked as Indeterminate and have not had the follow-up RNA testing necessary to know the individual's final HIV status.


## Hepatitis C Testing

The total number of Hepatitis C rapid tests conducted by Alyssa per month are shown in the plot below. This only considers the rapid tests initiated by our staff, not lab-initiated tests or blood draws (*unless it is the result of an initial rapid test*). The plot has the ability to zoom in on particular time frames by clicking and dragging. You can reset the graph by double-clicking within the plot.
...
monthly_hcv_testing_plot <- plot_ly(data = monthly_hcv_tests, x = ~Date, y = ~hcv_tests, type = 'bar', name = 'HCV Tests',
  ...
  layout(title = 'Wise Monthly Hepatitis C Testing',
         #xaxis = list(rangeslider = list(type = "date")),
...
Out of the total number of rapid HCV tests conducted by Alyssa (**n = `r hcv_rapid_tests`**), there were a total of [**`r hcv_rapid_positive` reactive/positive HCV results**]{style="color: orange;"} (*`r hcv_rapid_positive_percent` of all rapid HCV tests, `r hcv_rapid_positive`/`r hcv_rapid_tests`*). The total number of newly-identified positives from these tests was **`r hcv_rapid_new_positive` new HCV positive individuals** (*`r hcv_rapid_new_positive_percent` of all HCV positive HCV tests, `r hcv_rapid_new_positive`/`r hcv_rapid_positive`*). Therefore, the overall positivity rate for Alyssa's Hepatitis C testing is [**`r hcv_rapid_positivity_rate`**]{style="color: orange;"} (*`r hcv_rapid_new_positive`/`r hcv_rapid_tests`*).
...
HIV linkage cases are further stratified by those that are cases worked for individuals newly diagnosed versus those that were diagnosed previously. The HIV Linkage Flow Diagram is provided here for Alyssa's HIV linkage cases.
...
Therefore, Alyssa has **successfully linked a total of `r hcv_linked` HCV linkage cases** (*`r hcv_linked_percent` of all closed HCV linkage cases, `r hcv_linked`/`r hcv_closed_case`*). Out of all those cases that were successfully linked to care, `r hcv_new_linked` were for those newly diagnosed with HCV (*`r hcv_new_linked_percent` of all "eligible" HCV linkage cases for newly diagnosed individuals, `r hcv_new_linked`/`r hcv_new_eligible_ltc`*), and `r hcv_prev_linked` were for those that were previously diagnosed (*`r hcv_prev_linked_percent` of all "eligible" HCV linkage cases for previously diagnosed individuals, `r hcv_prev_linked`/`r hcv_prev_eligible_ltc`*).


#### SUD

The SUD Linkage Flow Diagram is provided here for Alyssa's SUD linkage cases.

Alyssa has worked on a total of **`r sud_ltc_cases` SUD linkage cases** (*`r sud_ltc_cases_percent` of all linkage cases, `r sud_ltc_cases`/`r ltc_cases`*).
...
Therefore, Alyssa has **successfully linked a total of `r sud_linked` SUD linkage cases** (*`r sud_linked_percent` of all closed SUD linkage cases, `r sud_linked`/`r sud_closed_case`*).


#### Human Trafficking

The Human Trafficking (HT) Linkage Flow Diagram is provided here for Alyssa's HT linkage cases.

Alyssa has worked on a total of **`r ht_ltc_cases` HT linkage cases** (*`r ht_ltc_cases_percent` of all linkage cases, `r ht_ltc_cases`/`r ltc_cases`*).
...
Therefore, Alyssa has **successfully linked a total of `r ht_linked` HT linkage cases** (*`r ht_linked_percent` of all closed HT linkage cases, `r ht_linked`/`r ht_closed_case`*).


#### Mental Health

The Mental Health (MH) Linkage Flow Diagram is provided here for Alyssa's MH linkage cases.

Alyssa has worked on a total of **`r mh_ltc_cases` MH linkage cases** (*`r mh_ltc_cases_percent` of all linkage cases, `r mh_ltc_cases`/`r ltc_cases`*).
...
Therefore, Alyssa has **successfully linked a total of `r mh_linked` MH linkage cases** (*`r mh_linked_percent` of all closed MH linkage cases, `r mh_linked`/`r mh_closed_case`*).


#### Housing

The Housing Linkage Flow Diagram is provided here for Alyssa's Housing linkage cases.

Alyssa has worked on a total of **`r housing_ltc_cases` Housing linkage cases** (*`r housing_ltc_cases_percent` of all linkage cases, `r housing_ltc_cases`/`r ltc_cases`*).
...
Therefore, Alyssa has **successfully linked a total of `r housing_linked` Housing linkage cases** (*`r housing_linked_percent` of all closed Housing linkage cases, `r housing_linked`/`r housing_closed_case`*).


#### PrEP

The PrEP Linkage Flow Diagram is provided here for Alyssa's PrEP linkage cases.

Alyssa has worked on a total of **`r prep_ltc_cases` PrEP linkage cases** (*`r prep_ltc_cases_percent` of all linkage cases, `r prep_ltc_cases`/`r ltc_cases`*).
...
Therefore, Alyssa has **successfully linked a total of `r prep_linked` PrEP linkage cases** (*`r prep_linked_percent` of all closed PrEP linkage cases, `r prep_linked`/`r prep_closed_case`*).
...
The STI Linkage Flow Diagram is provided here for Alyssa's STI linkage cases.

Alyssa has worked on a total of **`r sti_ltc_cases` STI linkage cases** (*`r sti_ltc_cases_percent` of all linkage cases, `r sti_ltc_cases`/`r ltc_cases`*).
...
Alyssa is currently working on `r sti_in_progress` of these STI linkage cases (*`r sti_in_progress_90days_percent` of which have been open for more than 90 days*), and therefore has closed a total of `r sti_closed_case` thus far (*`r sti_closed_case_percent` of all "eligible" STI linkage cases, `r sti_closed_case`/`r sti_eligible_ltc`*). Out of those cases closed, a total of `r sti_not_linked` were unable to be linked to care (*`r sti_not_linked_percent` of all closed STI linkage cases, `r sti_not_linked`/`r sti_closed_case`*), with the reasons as to why are shown in the flow diagram. These reasons are not mutually exclusive, and there could be multiple reasons why those linkage cases were unsuccessful.
...
Therefore, Alyssa has **successfully linked a total of `r sti_linked` STI linkage cases** (*`r sti_linked_percent` of all closed STI linkage cases, `r sti_linked`/`r sti_closed_case`*).


##### Food services

The Food services Linkage Flow Diagram is provided here for Alyssa's Food services linkage cases.

Alyssa has worked on a total of **`r food_ltc_cases` Food services linkage cases** (*`r food_ltc_cases_percent` of all linkage cases, `r food_ltc_cases`/`r ltc_cases`*).
...
Alyssa is currently working on `r food_in_progress` of these Food services linkage cases (*`r food_in_progress_90days_percent` of which have been open for more than 90 days*), and therefore has closed a total of `r food_closed_case` thus far (*`r food_closed_case_percent` of all "eligible" Food services linkage cases, `r food_closed_case`/`r food_eligible_ltc`*). Out of those cases closed, a total of `r food_not_linked` were unable to be linked to care (*`r food_not_linked_percent` of all closed Food services linkage cases, `r food_not_linked`/`r food_closed_case`*), with the reasons as to why are shown in the flow diagram. These reasons are not mutually exclusive, and there could be multiple reasons why those linkage cases were unsuccessful.
...
Therefore, Alyssa has **successfully linked a total of `r food_linked` Food services linkage cases** (*`r food_linked_percent` of all closed Food services linkage cases, `r food_linked`/`r food_closed_case`*).


##### Financial assistance

The Financial assistance Linkage Flow Diagram is provided here for Alyssa's Financial assistance linkage cases.

Alyssa has worked on a total of **`r financial_ltc_cases` Financial assistance linkage cases** (*`r financial_ltc_cases_percent` of all linkage cases, `r financial_ltc_cases`/`r ltc_cases`*).
...
Therefore, Alyssa has **successfully linked a total of `r financial_linked` Financial assistance linkage cases** (*`r financial_linked_percent` of all closed Financial assistance linkage cases, `r financial_linked`/`r financial_closed_case`*).


##### Person abuse

The Person abuse Linkage Flow Diagram is provided here for Alyssa's Person abuse linkage cases.

Alyssa has worked on a total of **`r personabuse_ltc_cases` Person abuse linkage cases** (*`r personabuse_ltc_cases_percent` of all linkage cases, `r personabuse_ltc_cases`/`r ltc_cases`*).
...
Therefore, Alyssa has **successfully linked a total of `r personabuse_linked` Person abuse linkage cases** (*`r personabuse_linked_percent` of all closed Person abuse linkage cases, `r personabuse_linked`/`r personabuse_closed_case`*).


##### General health

The General health Linkage Flow Diagram is provided here for Alyssa's General health linkage cases.

Alyssa has worked on a total of **`r general_ltc_cases` General health linkage cases** (*`r general_ltc_cases_percent` of all linkage cases, `r general_ltc_cases`/`r ltc_cases`*).
...
Therefore, Alyssa has **successfully linked a total of `r general_linked` General health linkage cases** (*`r general_linked_percent` of all closed General health linkage cases, `r general_linked`/`r general_closed_case`*).


##### Other Linkage 

The Other Linkage Flow Diagram is provided here for Alyssa's Other linkage cases.

Alyssa has worked on a total of **`r other_ltc_cases` Other linkage cases** (*`r other_ltc_cases_percent` of all linkage cases, `r other_ltc_cases`/`r ltc_cases`*).
...
Therefore, Alyssa has **successfully linked a total of `r other_linked` Other linkage cases** (*`r other_linked_percent` of all closed Other linkage cases, `r other_linked`/`r other_closed_case`*).
