...
enc$approach_assessment[(enc$counselor_risk_start == 'zlopz3el') &
...
# Declined
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments_declined[(enc$counselor_risk_start == 'zlopz3el') &
...
# Prevented
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments_prevented[(enc$counselor_risk_start == 'zlopz3el') &
                            enc$offer_outcome_risks == 2] <- 1
...
# Assessed
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments[(enc$counselor_risk_start == 'zlopz3el') &
...
table(enc$ctr_calc)
enc$ctr_tests[enc$ctr_calc == 1 & enc$counselor_hiv_start == 'zlopz3el'] <- 1
...
table(enc$ehe_calc)
enc$ehe_tests[enc$ehe_calc == 1 & enc$counselor_hiv_start == 'zlopz3el'] <- 1
ehe_tests <- enc[which(enc$ehe_calc == 1 & enc$counselor_hiv_start == 'zlopz3el'), ]
...
enc$hiv_false_positive[enc$counselor_hiv_start == 'zlopz3el' &
...
enc$hiv_indeterminate[enc$counselor_hiv_start == 'zlopz3el' & enc$final_test_status_hiv == 2] <- 1
...
enc$hcv_rapid_test[enc$counselor_hcv_start == 'zlopz3el' &
...
::: {.g-col-8}
This metrics report shows data for encounter information for **Zoe Lopez-Pintado**. 
Zoe's start and end date with EIP are shown below, however, any data contained 
