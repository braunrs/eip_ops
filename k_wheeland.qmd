...
### EIP Database Data
# 2023
eip23 <- read.csv('M:/EIP Data/EIP Database/REDCap 2023/EIP Database Data 20260126_1041.csv')
dim(eip23)
...
# 2024
eip24 <- read.csv('M:/EIP Data/EIP Database/REDCap 2024/EIP Database Data 20260126_1043.csv')
dim(eip24)
...
# 2025
eip25 <- read.csv('M:/EIP Data/EIP Database/REDCap 2025/EIP Database Data 20260126_1027.csv')
dim(eip25)
...
# 2026
eip26 <- read.csv('M:/EIP Data/EIP Database/REDCap 2026/EIP Database Data 20260126_1031.csv')
dim(eip26)
summary(eip26$record_id)
eip26$record_id <- paste(eip26$record_id, "_26", sep = "")
summary(eip26$record_id)

summary(eip26$enc_date)
eip26$enc_date <- as.Date(eip26$enc_date, format = '%m/%d/%Y')
summary(eip26$enc_date)
eip26 <- eip26[which(eip26$enc_date >= '2026-01-01'), ]
summary(eip26$enc_date)
...
enc$approach_assessment[(enc$counselor_risk_start == 'zwhek8tl') &
...
# Declined
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments_declined[(enc$counselor_risk_start == 'zwhek8tl') &
...
# Prevented
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments_prevented[(enc$counselor_risk_start == 'zwhek8tl') &
                            enc$offer_outcome_risks == 2] <- 1
...
# Assessed
table(enc$counselor_risk_start, enc$offer_outcome_risks)
enc$assessments[(enc$counselor_risk_start == 'zwhek8tl') &
...
table(enc$ctr_calc)
enc$ctr_tests[enc$ctr_calc == 1 & enc$counselor_hiv_start == 'zwhek8tl'] <- 1
...
table(enc$ehe_calc)
enc$ehe_tests[enc$ehe_calc == 1 & enc$counselor_hiv_start == 'zwhek8tl'] <- 1
ehe_tests <- enc[which(enc$ehe_calc == 1 & enc$counselor_hiv_start == 'zwhek8tl'), ]
...
enc$hiv_false_positive[enc$counselor_hiv_start == 'zwhek8tl' &
...
enc$hiv_indeterminate[enc$counselor_hiv_start == 'zwhek8tl' & enc$final_test_status_hiv == 2] <- 1
...
enc$hcv_rapid_test[enc$counselor_hcv_start == 'zwhek8tl' &
...
::: {.g-col-8}
This metrics report shows data for encounter information for **Kirsten Wheeland**. 
Kirsten's start and end date with EIP are shown below, however, any data contained 
